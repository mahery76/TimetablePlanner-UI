...
const LinkItem = ({ linkPath, linkTitle }) => {
  return (
    <Link
      href={linkPath}
      className="bg-blue-primary hover:bg-blue-secondary flex items-center justify-center p-2 rounded-md whitespace-nowrap"
    >
      {linkTitle}
    </Link>
  );
...
export default function timetableLayout({ children }) {
  return (
    <section>
      <div className="flex gap-x-5 mt-5 overflow-x-scroll md:justify-center">
        ...
        <LinkItem
          linkPath="/timetable_elements/teachers"
          linkTitle="Enseignants"
        />
        <LinkItem linkPath="/timetable_elements/rooms" linkTitle="Salles" />
        <LinkItem
          linkPath="/timetable_elements/timeslots"
          linkTitle="Créneaux horaires"
        />
        <LinkItem
          linkPath="/timetable_elements/core_classes"
